<template>
	<section>
		...
		<el-table :data="orders" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="orderId" label="订单号" width="250" sortable>
			</el-table-column>
			<el-table-column prop="basic.title" label="标题" width="250" sortable>
			</el-table-column>

			<el-table-column prop="extra.total" label="订单金额" :formatter="formatMoney" width="150" sortable>

			</el-table-column>
			<el-table-column prop="channel" label="支付方式" :formatter="formatPayment" width="150" sortable>
			</el-table-column>
			<el-table-column prop="created" label="创建时间" :formatter="formatDate" width="180" sortable>
			</el-table-column>
			<el-table-column prop="step" label="订单状态" :formatter="formatStep2" min-width="100" sortable>
			</el-table-column>
			<el-table-column label="操作" width="200">
				<template scope="scope">
				<el-button size="small" @click="handleShow(scope.$index, scope.row)">查看</el-button>
					<el-button type="primary" v-if="scope.row.step==2" size="small" @click="handleEdit(scope.$index, scope.row)">发货</el-button>
					<!--
		...
		<el-dialog title="发货" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="快递公司" prop="shipment.company">
		...
		<el-dialog title="订单详情" v-model="showFormVisible" :close-on-click-modal="false">
			...
			<el-col :span="18">
				¥{{ showForm.extra && showForm.extra.total.toFixed(2) }}
			</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
			<el-col :span="6">
			...
			<el-col :span="18">
				¥{{ showForm.amount && showForm.amount.toFixed(2) }}
			</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
				<el-col :span="6">
				...
				<el-col :span="18">
					¥{{ showForm.extra&&showForm.extra.couponPay?showForm.extra.couponPay.toFixed(2): 0.00 }}
				</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
				<el-col :span="6">
				...
				<el-col :span="18">
					¥{{ showForm.extra&&showForm.extra.cardPay?showForm.extra.cardPay.toFixed(2): 0.00 }}
				</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
			<div v-if='showForm.typehood === 8'>
				<el-col :span="6">
					商品清单:
				</el-col>
				<el-col :span="18">
			...
			<el-col :span="18" v-if='showForm.payment && showForm.payment.total_fee'>
				<p>¥{{  (showForm.payment.total_fee/100).toFixed(2) }}</p>
	...
	export default {
		...
		methods: {
			...
			formatMoney: function (row, column) {
				return row.extra.total?row.extra.total.toFixed(2):0
			},
			formatTime( time ) {
			...
			formatPayment(row){
				const payments = {'wewap': '微信公众号支付'}
				return payments[row.channel]
			},
