<template>
	<section>
		...
		<el-table :data="orders" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column label='订单状态' width="100">
				<template scope='scope'>
					...
					<el-tag type="primary" v-if="scope.row.step==2">待服务</el-tag>
					<el-tag type="warning" v-if="scope.row.step==3">待完成</el-tag>
			...
			<el-table-column label="操作" width="200">
				...
				<el-button size="small" @click="handleShow(scope.$index, scope.row)">查看</el-button>
					<el-button v-if="scope.row.step==2" type="primary" size="small" @click="handleEdit(scope.$index, scope.row)">服务</el-button>
		...
		<!--编辑界面-->
		<el-dialog title="订单服务" v-model="editFormVisible" :close-on-click-modal="false">
			<el-form :model="editForm" label-width="80px" :rules="editFormRules" ref="editForm">
				...
				<el-form-item label="商品清单">
					<el-row v-for="p in editForm.basic.products" :key='p.count'>
						<el-col :span="4">
						{{p.basic.title}}
						</el-col>
						<el-col :span="12">
			...
			<div slot="footer" class="dialog-footer">
				...
				<el-button type="primary" @click.native="editSubmit" :loading="editLoading">确认完成</el-button>
			</div>
		</el-dialog>
		...
		<el-dialog title="订单详情" v-model="showFormVisible" :close-on-click-modal="false">
			...
			<el-col :span="18">
				<p v-for="p in (showForm.basic && showForm.basic.products)" :key='p.count'>{{ p.basic.title }} {{p.spec}} x{{ p.count }}</p>
			</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
			<el-col :span="6">
			...
			<el-col :span="18">
				<p>姓名：{{ showForm.basic && showForm.basic.address.name }}</p>
				<p>手机号：{{ showForm.basic && showForm.basic.address.phone }}</p>
			</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
			<el-col :span="6">
			...
			<el-col :span="18">
				<p>支付金额：¥{{ showForm.payment && (showForm.payment.total_fee/100).toFixed(2) }}</p>
				<p>支付时间：{{ formatTime(showForm.payment && showForm.payment.created) }}</p>
			</el-col>
			</el-row>
			<el-row style="margin-top:5px;" v-if="showForm.step >= 3">
			<el-col :span="6">
				服务信息:
			</el-col>
			<el-col :span="18">
				
				<p>服务时间：{{ formatTime(showForm.shipment && showForm.shipment.created) }}</p>
			</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
			<el-col :span="6">
			...
			<el-col :span="18">
				<p><img :src="showForm.people && showForm.people.avatar" class="avatar" /></p>
...
<script>
	...
	export default {
		...
		methods: {
			...
			formatStep( step ) {
				const steps = ['待支付', '待服务', '待完成', '已完成', '已退款', '已取消']
				return steps[step-1]
			},
