<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					...
					<el-button v-on:click="handleAdd">新增活动</el-button>
				</el-form-item>
			</el-form>
		</el-col>
		...
		<!--编辑界面-->
		<el-dialog title="修改活动" v-model="editFormVisible" :close-on-click-modal="false">
		...
		<!--显示详情-->
		<el-dialog title="活动详情" v-model="showFormVisible" :close-on-click-modal="false">
			<el-row class="el-row">
			<el-col :span="6">
				活动ID:
			</el-col>
...
<script>
	...
	import { getFaceActivityListPage, addFaceActivity, editFaceActivity, removeFaceActivity, findActivityQrcode } from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getProducts() {
				...
				//NProgress.start();
				getFaceActivityListPage(para).then((res) => {
					if(res.code){
						this.$message.error({
							message: res.message
						})
			...
			handleDel: function (index, row) {
				this.$confirm(`确认删除活动：${row.basic.title} 吗?`, '提示', {
					type: 'warning'
				}).then(() => {
					this.listLoading = true;
					//NProgress.start();
					...
					removeFaceActivity(para).then((res) => {
						this.listLoading = false;
			...
			handleShow: function (index, row) {
				...
				findActivityQrcode({feedId: row.feedId}).then(res => {
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							editFaceActivity(para).then((res) => {
