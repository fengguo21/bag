...
<script>
	...
	import { getAppListPage, addApp, getApp, editAppWepayFacilitator, getPeopleListPage, createRole, getMasterMenu, addMenu, editApp, avaterUrl } from '../../api/api';
	...
	export default {
		...
		methods: {
			...
			getUsers() {
				...
				//NProgress.start();
				getAppListPage(para).then((res) => {console.log(res)
					if(res.code){
						this.$message.error({
							message: res.message
						})
