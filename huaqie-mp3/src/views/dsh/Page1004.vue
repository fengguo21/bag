	...
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true" :model="filters">
				...
				<el-form-item>
					<el-select v-model="filters.typehood" :clearable="true" placeholder="订单类型" clearable>
					    <el-option
				...
				<el-form-item>
					<el-select v-model="filters.step" :clearable="true" placeholder="付款状态" clearable>
					    <el-option
		...
		<el-table :data="orders" highlight-current-row v-loading="listLoading" @selection-change="selsChange" style="width: 100%;">
			...
			<el-table-column prop="created" label="创建时间" :formatter="formatDate" min-width="150" >
			</el-table-column>			
		...
		<el-dialog title="订单详情" v-model="showFormVisible" :close-on-click-modal="false">
			...
			<el-col :span="18">
				{{ (showForm.payment && showForm.payment.created)?formatTime(showForm.payment.created):'' }}
			</el-col>
			</el-row>
			...
			<el-col :span="18">
				<p v-if="showForm.payment && showForm.payment.total_fee">支付金额：¥{{ showForm.payment && (showForm.payment.total_fee/100).toFixed(2) }}</p>
			...
			<el-col :span="18">
				<p v-if="showForm.extra && showForm.extra.couponId">{{ showForm.extra && showForm.extra.couponId }}</p>
			</el-col>
			</el-row>
			<el-row style="margin-top:5px;">
			<el-col :span="6">
	...
	export default {
		...
		methods: {
			...
			formatTypehood: function (row, column) {
				return (row.typehood == 12)?'购买书籍':'购买VIP'
			},
